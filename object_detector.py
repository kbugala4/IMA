...
def process(score, class_id, class_name, mask, xyz, trans):
    ...
    # cen = np.matmul(R, trans.transform.translation) + T
    # # przekształcenie orientacji znaku
    # ori = np.matmul(R, ori)

    # # publikacja transformacji, tym razem w układzie bazowym (odom)
    # c=R*cen+transformed_z

    # print("translation:")
    # print(c)
    # transformed_x = c[0]+trans.transform.translation.x
    # transformed_y = c[1]+trans.transform.translation.y
    # transformed_z = c[2]+trans.transform.translation.z
    # pose_transformed = [transformed_x, transformed_y, transformed_z]

    #rospy.loginfo("transform at %f, %f, %f",trans.transform.translation.y+c[0], c[1]+trans.transform.translation.z, c[2]+trans.transform.translation.x)


    marker_ = Marker()
    marker_.header.frame_id = "/kinect_rgb_optical_frame"
